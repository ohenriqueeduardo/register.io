...
import { ArrowRight, Building2, Lock, Mail, User, AlertCircle } from "lucide-react";
...
// Extende registerSchema do zod para incluir a validação de confirmação de senha no frontend
const frontendRegisterSchema = registerSchema
  .extend({
...
type RegisterFormData = z.infer<typeof frontendRegisterSchema>;
...
export default function Register() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      await authService.register(data.nome, data.email, data.password);
      ...
      <section className="hidden bg-slate-900 dark:bg-slate-950 p-12 text-slate-100 lg:flex lg:flex-col lg:justify-between border-r border-slate-800 relative overflow-hidden">
        ...
        <div className="max-w-lg relative z-10">
          <h1 className="text-5xl font-extrabold leading-tight tracking-tight text-white mb-6">
            Cadastre-se na nossa plataforma administrativa.
          </h1>
          <p className="text-lg text-slate-300 leading-relaxed">
            Seja bem-vindo ao hub de controle de cadastros. Proteção avançada de hash de senha e ambiente responsivo para qualquer tamanho de tela.
          </p>
        </div>
        ...
        <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-950 p-8 shadow-xl border border-slate-100 dark:border-slate-700/80 transition-colors duration-200">
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm font-medium text-slate-600 dark:text-slate-300">
              Preencha os dados abaixo para criar o seu acesso ao painel.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="nome" className="font-semibold text-slate-700 dark:text-slate-300">
                Nome Completo
              </Label>
